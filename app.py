...
def graph(id):
    ...
    data2 = [(str(sprint_start.date()),counter2)]
    completed_story_point = 0
    x_values = []
    remove_date = []
    
    for cards in get_cards:
        if (cards[6] == "Done") and (cards[12] == id) and (datetime.datetime.strptime(cards[9], "%Y-%m-%d") <= sprint_end):
            
            completed_story_point += int(cards[4])
            add = (cards[9], counter2 - completed_story_point)
            data2.append(add)
            
    
    data2.append((str(sprint_end.date()),counter2 - completed_story_point))
    
    for j in range(len(data2)):
        if j+1 < len(data2):
            if data2[j][0] == data2[j+1][0] and data2[j][1] > data2[j+1][1]:
                
                remove_date.append(data2[j])
    
    for k in range(len(remove_date)):
        data2.remove(remove_date[k])
    
    for t in range(len(data2)):
        data2[t] = list(data2[t])
        
    
    
    for i in range(len(data1)):
                  
        x_values.append(str(sprint_start.date())) 
        sprint_start+=(diff/(counter))    
        for p in range(len(data2)):
            if data2[p][0] not in x_values and (datetime.datetime.strptime(data2[p][0], "%Y-%m-%d").date() < sprint_start.date()):

                
                x_values.append(data2[p][0])

                
    x_values.append(str(sprint_end.date()))       
    labels = x_values
    values = [row[1] for row in data2]
    x = [row[0] for row in data2]
    v = (str(sprint_end.date()))
    return render_template("graph.html", labels=labels, values=values, counter2 = counter2, v = v, x = x)
